...
public class AbilityCooldownUI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        bool volleyBlockedByAirLift = IsVolleyBlockedByAirLift();

        float targetFill = volleyBlockedByAirLift ? 0f : GetFill();
        displayedFill = Mathf.MoveTowards(displayedFill, targetFill, fillSmoothSpeed * Time.deltaTime);

        bool ready = !volleyBlockedByAirLift && GetReady();
        bool active = !volleyBlockedByAirLift && GetActive();
        ...
        UpdateGlow(ready && !volleyBlockedByAirLift, active);
        UpdateLabels(ready, active, volleyBlockedByAirLift);
        RefreshVolleyChrome(volleyBlockedByAirLift);
    }

    bool IsVolleyBlockedByAirLift()
    {
        if (abilityType != AbilityBarType.Volley || volley == null) return false;
        PlayerThrow throwScript = volley.GetComponent<PlayerThrow>();
        return throwScript != null && throwScript.AirLiftModeSelected;
    }
    ...
    void RefreshVolleyChrome(bool blockedByAirLift)
    {
        if (abilityType != AbilityBarType.Volley) return;

        if (labelText != null)
        {
            labelText.text = blockedByAirLift ? "AIRLIFT" : GetLabel();
        }

        if (keyHintText != null)
        {
            keyHintText.text = blockedByAirLift ? "—" : GetKeyHint();
        }
    }
    ...
    void UpdateLabels(bool ready, bool active, bool volleyBlockedByAirLift = false)
    {
        Color target = volleyBlockedByAirLift
            ? new Color(0.45f, 0.45f, 0.5f, 0.75f)
            : (ready && !active ? labelReadyColor : labelCooldownColor);
