...
public class ThrowerBot : MonoBehaviour
{
    ...
    [Header("Reposition")]
    [Tooltip("Atislar arasinda thrower bolgesi icinde yer degistir.")]
    public bool repositionBetweenThrows = true;
    public float repositionSpeed = 3.8f;
    public float repositionArriveDistance = 0.55f;

    private PlayerThrow playerThrow;
    private PlayerHealth playerHealth;
    private Rigidbody rb;
    private RoleZoneLimiter zoneLimiter;
    ...
    void FixedUpdate()
    {
        if (!repositionBetweenThrows || !hasMoveTarget || isTelegraphing)
        {
            return;
        }
        ...
        if (toTarget.magnitude <= repositionArriveDistance)
        {
            hasMoveTarget = false;
            StopHorizontalMotion();
            return;
        ...
        Vector3 velocity = toTarget.normalized * repositionSpeed;
    ...
    void PickMoveTarget()
    {
        ...
        moveTarget = new Vector3(
            Random.Range(bounds.min.x + pad, bounds.max.x - pad),
            transform.position.y,
            Random.Range(bounds.min.z + pad, bounds.max.z - pad)
        );
        hasMoveTarget = true;
    }
